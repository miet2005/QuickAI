...
const BlogTitles = () => {
  ...
  const onSubmitHandler= async (e)=>{
    ...
    try {
      setLoading(true)
      const prompt = `Give a blog title on the topic  ${input} in category  ${selectedCategory}`
      const {data} = await axios.post('api/ai/blog-title',
        {
          prompt
        },
        {
    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        <div className='flex items-center gap-4'>
          ...
          <button disabled={loading} className='w-full flex justify-center items-center gap-2 bg-gradient-to-r  from-[#8E37EB] to-[#C341F6] text-white px-4 py-2 mt-6 text-sm rounded-lg cursor-pointer'>
