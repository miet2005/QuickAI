...
const GenerateImages = () => {
    ...
    const style=['Realistic', 'Ghibli', 'Anime', 'Cartoon', 'Fantasy', '3D', 'Portrait']
  ...
  const {getToken} = useAuth()
    ...
    try {
      setLoading(true)
      const prompt = `Generate an image on the basis of the description given : ${input} in the selected style :  ${selectedStyle}`
...
<div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        <div className='flex items-center gap-4'>
          ...
          <textarea onChange={(e)=>setInput(e.target.value)} value={input} rows={8} className='w-full  p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300' placeholder='Describe what you want to see in the image..' required />
